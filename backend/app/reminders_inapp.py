...
# ─────────────────────────────────────────────────────────
# MODELS  (separate Base so they don't conflict with existing)
# ─────────────────────────────────────────────────────────
...
class MedicineReminder(ReminderBase):
    ...
    reminder_times = Column(Text, nullable=False)   # JSON string
...
def get_reminders(
    ...
    for r in reminders:
        ...
        result.append(reminder_to_dict(r, today_status=log.status if log else None))
...
def update_reminder(
    ...
    reminder = (
        db.query(MedicineReminder)
        .filter(MedicineReminder.id == rid, MedicineReminder.user_id == user["id"])
        .first()
    )
    if not reminder:
        raise HTTPException(404, "Reminder not found")

    if body.medicine_name  is not None: reminder.medicine_name  = body.medicine_name
    if body.dosage         is not None: reminder.dosage         = body.dosage
    if body.frequency      is not None: reminder.frequency      = body.frequency
    if body.notes          is not None: reminder.notes          = body.notes
    if body.is_active      is not None: reminder.is_active      = body.is_active
    if body.end_date       is not None: reminder.end_date       = body.end_date
    if body.reminder_times is not None: reminder.reminder_times = json.dumps(body.reminder_times)
...
def log_dose(
    ...
    db.add(log)
    db.commit()
    return {"id": log.id, "message": f"Logged as {body.status}"}
...
def dose_history(
    ...
    logs = (
        db.query(DoseLogModel)
        .filter(DoseLogModel.reminder_id == rid, DoseLogModel.user_id == user["id"])
        .order_by(DoseLogModel.logged_at.desc())
        .limit(30)
        .all()
    )
